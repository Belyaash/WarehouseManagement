...
namespace Application.Features.LoadingDocuments.Command.InsertLoadingDocument;
...
file sealed class InsertLoadingDocumentHandler : IRequestHandler<InsertLoadingDocumentCommand, InsertLoadingDocumentResponseDto>
{
    ...
    private async Task LoadExistingBalancesAsync(InsertLoadingDocumentCommand request, CancellationToken cancellationToken)
    {
        ...
        await _context.MeasureUnits
            .Include(r => r.Balances)
            .Where(MeasureUnit.Spec.ByIdsList(request.Dto.DocumentResources.Select(dr => dr.MeasureUnitId)))
            .LoadAsync(cancellationToken);
    }

    private LoadingDocument CreateDocument(InsertLoadingDocumentCommand request)
    {
        ...
        var newDocument = new LoadingDocument(createParameters);
    ...
    private void CreateDocumentResources(InsertLoadingDocumentCommand request, LoadingDocument newDocument)
    {
