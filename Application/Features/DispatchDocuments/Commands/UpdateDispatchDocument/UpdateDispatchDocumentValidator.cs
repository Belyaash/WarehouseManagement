...
public sealed class UpdateDispatchDocumentValidator : AbstractValidator<UpdateDispatchDocumentCommand>
{
    ...
    private Task<bool> IsValueUniqueAsync(UpdateDispatchDocumentRequestDto dto, CancellationToken cancellationToken)
    {
        return _context.DispatchDocuments
            .Where(!DispatchDocument.Spec.ById(dto.Id))
            .AllAsync(!DispatchDocument.Spec.ByNumber(dto.DocumentNumber.Trim()), cancellationToken);
    }
    ...
    private static bool CanUpdateOldResources(UpdateDispatchDocumentRequestDto dto, DispatchDocument document)
    {
